...
func ParentProcessSendConfigViaSTDIN(config *commons.ServerConfig, stdin io.WriteCloser, stdout io.ReadCloser) error {
	logger := log.WithFields(log.Fields{
		"package":  "commons",
...
func ChildProcessReadConfigViaSTDIN() (*commons.ServerConfig, io.WriteCloser, error) {
	logger := log.WithFields(log.Fields{
		"package":  "commons",
	...
	config, err := commons.NewServerConfigFromYAML(configBytes)
	if err != nil {
		logger.WithError(err).Error("failed to read configuration")
		return nil, nil, err
	}
