...
import (
	...
	cmd_commons "github.com/cyverse/irods-rule-async-exec-cmd/server-cmd/commons"
	"github.com/cyverse/irods-rule-async-exec-cmd/service"
...
var rootCmd = &cobra.Command{
	Use:   "irods-rule-async-exec-svc [args..]",
	Short: "Run iRODS Rule Async Exec Service",
	Long:  "Run iRODS Rule Async Exec Service that handles requests from clients.",
	RunE:  processCommand,
}
...
func parentMain(command *cobra.Command, args []string) {
	...
	if !config.Foreground {
		...
		if err != nil {
			logger.WithError(err).Error("failed to run iRODS Rule Async Exec Cmd Service child process")
			os.Exit(1)
		}

		err = cmd_commons.ParentProcessSendConfigViaSTDIN(config, childStdin, childStdout)
		if err != nil {
			logger.WithError(err).Error("failed to send configuration to iRODS Rule Async Exec Cmd Service child process")
			os.Exit(1)
		}
	} else {
		// run foreground
...
// run runs iRODS Rule Async Exec Cmd Service
func run(config *commons.ServerConfig, isChildProcess bool) error {
	logger := log.WithFields(log.Fields{
		"package":  "main",
		"function": "run",
	})
